...
분할 진입하고, 다음 거래일 아침 기술적 반등(+5~10%)에 청산하는 것. 이 phase는
`SCAN`/`INTRADAY`와 같은 **REVERSAL 전술**을 장후반 종가 매매 버전으로 실행한다.

## 절대 원칙
1. **잡주 금지.** 최근 20일 내 하루 거래대금 2,000억원 이상 터진 이력이 있는, 시장의
   관심을 크게 받았던 주도주만 대상이다. 이력이 확인되지 않으면 제외.
2. **극단적 과매도만.** 20일 이평선 대비 이격도 80% 이하(이격도 -20%) + 당일 -7% 이상 폭락
   수준의 처절한 투매여야 한다. SCAN phase의 REVERSAL 기준(-15%)보다 의도적으로 더 엄격하다
   — 장 후반이라 추가 투매 위험이 크기 때문이다. 어중간한 하락(-3~-5%)은 다음 날 더 빠질 수 있다.
3. **작은 사이즈.** 이 셋업은 떨어지는 칼을 잡는 것이다 — confidence는 보수적으로,
   레짐이 어떻든 실제 낙폭·거래대금·반등 구조가 약하면 진입하지 말라.
...
## stop_loss_price 산정 원칙
**반드시 실제 가격을 입력하라. 0은 미정이 아니라 오류다.**
기본 기준: `get_ohlcv` 또는 `get_intraday_candles`로 확인한 **당일 장중 저점 × 0.98**
(저점 -2%). 투매 클라이맥스가 있었다면 그 저점이 자연스러운 손절 기준이 된다.
직전 지지선(20일선 등)이 더 낮다면 그 가격을 쓴다.
어느 방법으로든 계산 불가하면 진입가 × 0.95(진입가 -5%)를 최후 기준으로 사용하라.
...
- `stop_loss_price: 0` 제출 금지 — 반드시 위 원칙으로 계산한 실제 가격을 기재.
