...
2. `get_market_context`로 지수 등락률 확인 — 코스피/코스닥 -3% 이하면 **폭락일 판단**
   ...
   → `trading_value_top`에서 `change_pct <= -5%` 개별 종목 추출 → REVERSAL_BOTTOM 탐색
...
4. `volume_surge_candidates` 검증 (VOLUME_SURGE_LEADER / THEME_CATALYST):
   ...
   - 테마 내 거래대금 1위인지 확인
5. **폭락일 한정** — REVERSAL_BOTTOM 탐색 (구체적 순서 준수):
   ...
   **Step C — 패닉셀 강도 검증**
   ...
   오늘이 5일 평균의 **2배 이상**이면 패닉셀 확정 → REVERSAL_BOTTOM으로 등재.
...
- **REVERSAL_BOTTOM**: `get_disparity_rank`(d20 이격도)로 탐색 금지 — `trading_value_top`의 `change_pct <= -5%`가 유일한 진입점
- **REVERSAL_BOTTOM**: 오늘 거래대금이 5일 평균 2배 미만이면 등재 금지
